...
export function createPortIpcMain(port: LabPort): {
  ...
  port.addEventListener("message", async event => {
    ...
    if (!handler) {
      port.postMessage({ kind: "ipc-result", id: message.id, ok: false, error: `No IPC handler for ${(message as InvokeMessage).channel}` });
      return;
    }
    try {
      const value = await handler({ sender: "lab-host" }, ...((message as InvokeMessage).args ?? []));
...
export function postFeed(port: LabPort, row: FeedRow): void {
...
export class PortApiClient {
  ...
  #route(message: unknown): void {
    ...
    if (msg?.kind === "ipc-result") {
      ...
      if (!task) throw new Error(`Lab IPC: result for unknown call ${msg.id}.`);
