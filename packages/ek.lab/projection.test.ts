...
test("placed orders pend until the seller admits them", () => {
  const placed: EkOrder = { $type$: "EkOrder", department: DEPT, idempotencyKey: "p1", customer: CUSTOMER, seller: CUSTOMER, offer: "o1", quantity: 2, lot: "ek-lot-a", facility: "ek-facility", currency: "EUR", unitAmount: 10000, admittedAt: 0 };
  ...
  const stocked: EkStockReceipt[] = [{ $type$: "EkStockReceipt", department: DEPT, receiptId: "r1", lot: "ek-lot-a", facility: "ek-facility", quantity: 2, receivedBy: ADMIN, receivedAt: 1 }];
...
test("admission accrues receivables and payables per role", () => {
  const placed: EkOrder = { $type$: "EkOrder", department: DEPT, idempotencyKey: "m1", customer: CUSTOMER, seller: CUSTOMER, offer: "o1", quantity: 2, lot: "ek-lot-a", facility: "ek-facility", currency: "EUR", unitAmount: 10000, admittedAt: 0 };
  ...
  const mine = projectDepartment({ department, assignments, contacts: [], offers: [], orders: [admitted], stock: [], viewer: CUSTOMER, atTime: 10 });
...
test("projection rejects unauthorized rows and scopes customer reads", () => {
  const order: EkOrder = { $type$: "EkOrder", department: DEPT, idempotencyKey: "k1", customer: CUSTOMER, seller: SELLER, offer: "o1", quantity: 3, lot: "ek-lot-a", facility: "ek-facility", currency: "EUR", unitAmount: 10000, admittedAt: 2 };
...
test("admin authority is never delegated down the chain", () => {
  ...
  const forgedOffer: EkOffer = { $type$: "EkOffer", department: DEPT, offerId: "coup", item: "x", priceList: "p", channel: "facility", unitAmount: 1, currency: "EUR", publishedBy: CUSTOMER };
  ...
  assert.deepEqual(view.offers, []);
  assert.deepEqual(view.rejected, [{ type: "EkOffer", id: "coup", reason: "publisher-not-authorized" }]);
});

test("concurrent admissions settle deterministically, never negative", () => {
  const stock: EkStockReceipt[] = [{ $type$: "EkStockReceipt", department: DEPT, receiptId: "r1", lot: "ek-lot-a", facility: "ek-facility", quantity: 1, receivedBy: ADMIN, receivedAt: 1 }];
  const first: EkOrder = { $type$: "EkOrder", department: DEPT, idempotencyKey: "race-a", customer: CUSTOMER, seller: SELLER, offer: "o1", quantity: 1, lot: "ek-lot-a", facility: "ek-facility", currency: "EUR", unitAmount: 10000, admittedAt: 9 };
  const second: EkOrder = { ...first, idempotencyKey: "race-b", admittedAt: 10 };
  ...
  assert.deepEqual(view.rejected, [{ type: "EkOrder", id: "race-b", reason: "oversold" }]);
...
test("customer contacts stay with their seller", () => {
  const sellerContact: EkContact = { $type$: "EkContact", department: DEPT, person: SELLER, name: "Seller", role: "seller", publishedBy: SELLER, publishedAt: 2 };
  const customerContact: EkContact = { $type$: "EkContact", department: DEPT, person: CUSTOMER, name: "Customer", role: "customer", publishedBy: CUSTOMER, publishedAt: 2 };
...
test("customers project their own admissions without the receipts", () => {
  const admitted: EkOrder = { $type$: "EkOrder", department: DEPT, idempotencyKey: "mine", customer: CUSTOMER, seller: SELLER, offer: "o1", quantity: 1, lot: "ek-lot-a", facility: "ek-facility", currency: "EUR", unitAmount: 10000, admittedAt: 9 };
