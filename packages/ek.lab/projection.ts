...
export function projectDepartment({ department, assignments, contacts, offers, orders, purchaseRequests = [], purchaseDecisions = [], stock, viewer, atTime }: {
  ...
  const authorizedRequests = purchaseRequests.filter(entry => {
    ...
    return Boolean(matchingOrder) && ownsCustomer &&
      admit("purchase-request", "EkPurchaseRequest", entry.idempotencyKey, entry.customer, entry.customer);
  });
  const authorizedDecisions = purchaseDecisions.filter(entry => {
    ...
    rejected.push({ type: "EkPurchaseDecision", id: entry.idempotencyKey, reason: "publisher-not-authorized" });
    return false;
  });
  ...
  return {
    ...
    contacts: contacts.filter(entry => {
      if (!admit("contact", "EkContact", entry.person, entry.publishedBy, entry.person)) return false;
