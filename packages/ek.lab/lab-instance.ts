// packages/ek.lab/lab-instance.ts
/**
 ...
 * the `eklab:` dialer and the feed-forward stream on the given port.
 */
...
export const labUrl = (key: string): string => `eklab://${key}`;
...
export async function startLabInstance({ port, key, email, secret, directory, createMessageChannel }: LabInstanceOptions): Promise<{ shutdown(): Promise<void> }> {
  ...
  const multiUser = new MultiUser({
    ...
    recipes: [...RecipesStable, ...RecipesExperimental, ...AccessRightsRecipes, ...(EkLabRecipes as unknown as Recipe[])],
    reverseMaps: merge(ReverseMapsStable, ReverseMapsExperimental),
    reverseMapsForIdObjects: merge(ReverseMapsForIdObjectsStable, ReverseMapsForIdObjectsExperimental, EkLabReverseMapsForIdObjects),
  });
  await multiUser.loginOrRegister(email, secret, key);
  await objectEvents.init();

  // The instance endpoint published in our profile is the lab URL: peers'
  // outgoing routes dial it through the `eklab:` dialer below.
  ...
  const connections = new ConnectionsModel(leuteModel, {
    ...
    // catchAll pre-registers this worker's credential for the eklab:// listener at
  ...
  const chatPlan = createChatPlan({
    ...
    notify: peer => postFeed(port, { type: "EkChat", id: peer, hash: peer, kind: "chat" }),
  });
  ...
  registry.register("ekChat", chatPlan, {
    description: "Ek lab 1:1 chat over topic channels",
