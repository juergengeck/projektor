 ...
 * could dial directly (browsers cannot listen, and the mesh `eklab://`
...
function relayHttpOrigin(relayUrl: string): string {
  ...
  if (url.protocol !== "ws:" && url.protocol !== "wss:") {
    throw new Error("Ek lab: IoM rendezvous must be a ws:// or wss:// URL.");
  }
...
export interface IoMDeps {
  ...
  self(): string;
  /** Listener id carrying the registered pairing credential (eklab:// url). */
  listenerUrl: string;
...
function waitOpen(socket: WebSocket, url: string, timeoutMs: number): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      socket.close();
      done(() => reject(new Error(`Ek lab: rendezvous unreachable (${url}).`)));
    }, timeoutMs);
    unref(timer);
    const onOpen = (): void => done(resolve);
    const onError = (): void => done(() => reject(new Error(`Ek lab: rendezvous unreachable (${url}).`)));
...
export function createIoMOps({ connections, self, listenerUrl, email }: IoMDeps) {
  ...
  return {
    ...
    async awaitIoMInvite({ token, timeoutMs = 120_000 }: {
      ...
      if (!pending) throw new Error("Ek lab: unknown IoM invitation token.");
      await withTimeout(pending.paired, timeoutMs, "Ek lab: IoM pairing timed out waiting for the second device.");
