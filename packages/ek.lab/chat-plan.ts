...
export function createChatPlan({ topicModel, channelManager, self, notify }: {
  ...
  const checkPeer = (peer: string): SHA256IdHash<Person> => {
    if (typeof peer !== "string" || !HASH.test(peer)) throw new Error("Ek lab: chat peer must be a SHA-256 hash.");
    if (peer === self()) throw new Error("Ek lab: cannot chat with yourself.");
    return peer as SHA256IdHash<Person>;
  };

  const checkText = (text: string): string => {
    if (typeof text !== "string" || text.trim() === "") throw new Error("Ek lab: message text is required.");
    if (text.length > 2000) throw new Error("Ek lab: message text must be at most 2000 characters.");
    return text;
  };
