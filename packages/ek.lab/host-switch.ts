// packages/ek.lab/host-switch.ts
/**
 * The web host's only job between workers: switch `eklab:` dials to the
 * target worker's port. Shared by the node integration test and the browser
 * LabHost; it never reads Ek data.
 */
    ...
    async pairAll() {
      for (let i = 0; i < keys.length; i += 1) {
        for (let j = i + 1; j < keys.length; j += 1) {
          ...
          // The mesh stays on the local eklab:// switch (hermetic and
          // offline-capable): worker invitations now carry the commserver URL
          // for IoM, so pairAll redials them at the eklab:// listener instead.
