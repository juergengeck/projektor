...
Every column has independent navigation, selected department and session state. Identity and scope cannot use the current unqualified localStorage keys shared by the whole origin. A sticky column header shows actor, effective role, connection state and current projection/root version. The shell highlights a projection only after that worker reports a meaningful local or remote change. It distinguishes local commit, publication/share, peer receipt, admission or denial, projection change and business acceptance; it never calls an asynchronous peer update an atomic distributed commit. Lane buttons stay disabled until the worker's own projection shows the action can succeed.

Appointments follow the chain admin appoints managers, managers appoint sellers, sellers appoint customers; the root admin keeps universal authority. assignRole gates each link explicitly and rolesOf re-validates the chain on every read, so a forged assignment lands in rejected instead of silently counting. The lane offers each appointment exactly where the chain allows it.

Disclosure follows the chain, never broadcasts. A published offer reaches sellers through their manager only; the shareOffer operation (seller or root admin, appointed customer only) is the sole way inventory reaches customers. A customer places an order for themselves (intent only, admittedAt 0, stock untouched); the seller admits the exact placed key, recording seller and admittedAt as a new version under the same identity. Nobody buys without placing first: admitting an unknown or already-admitted key fails, and placing as a non-customer fails. Who sees what:

| Artifact | Admin | Manager | Seller | Customer |
|---|---|---|---|---|
| Department, assignments, contacts | yes | yes | yes | yes |
| Published offer | yes | yes | yes | only via seller shareOffer |
| Placed order (admittedAt 0) | yes | no | yes | own only |
| Admitted order | yes | yes | yes | own only |

The demo Buy button calls buy, which records a typed purchase request alongside the placed order. The appointed seller processes that request automatically in its own worker, applies the serialized stock check, and publishes the confirmed order. An out-of-stock decision reaches the customer as a failed purchase and never reduces availability or creates a payable. The manual placeOrder and admitOrder operations remain available for explicit protocol ceremonies.

Availability counts admitted purchases only. Appointment membership changes keep each row's own audience instead of re-granting every row to everyone: a new appointment can never leak one level's rows to another.

Each app automatically displays a same-person device invitation below its fixed frame. The lane URL carries an encodeURIComponent JSON fragment with token, Glue commserver URL, publicKey, protocol version, primed mode, same-person relation and deviceEnrollmentPersonId. Scanning the QR opens the lane's join flow; standard ONE pairing through wss://api.glue.one/comm enrolls the second device as that person. Acceptance fails fast for any other person. The QR can be enlarged or its link copied, and it clears when its exact invitation completes.

The proving scenario is: admin appoints manager, manager appoints seller, seller appoints customer; the manager publishes an authorized offer; the seller receives it and shares it down; the customer buys; the seller worker automatically admits the exact placed order; the customer holds the admitted purchase while availability draws down; the manager observes the reservation and inventory effect; and the country-organisation admin observes only journal evidence permitted by policy, not private payload. The scenario records both propagation and intentional non-propagation: publishing alone shares nothing with customers, and placing alone buys nothing. A reset creates four clean, distinct instance identities and leaves the normal single-instance /amway workspace available. Persisted peer endpoints are converted back into explicit connection demand during worker startup, so a reload restores the live CHUM mesh before readiness instead of stranding subsequent purchases in the origin column. The integration suite also proves pause/resume catch-up and that a customer self-purchase feeds the same admitted order version back to admin, manager and seller projections.
