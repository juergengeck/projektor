...
const index = createProjectFileIndex({
  ...
  files: [
    { path: "projects/demo-kita-2028/project.json", kind: "project", status: "tracked", phase: "LP3" },
...
assert.equal(index.files[0].label, "project.json");
...
assert.throws(
  () => createProjectFileIndex({ source, files: [{ path: "/absolute/project.json" }] }),
  /repository-relative/,
);

assert.throws(
  () => createProjectFileIndex({ source, files: [{ path: "project.json", status: "synced-ish" }] }),
...
assert.equal(gitBacked.index.files.find((file) => file.path === "README.md").contentHash, "dirty-readme");
