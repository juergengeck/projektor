...
test("every department-scoped type is reverse-mapped on department", () => {
  ...
  for (const type of AMWAY_LAB_TYPES.filter(name => name !== "AmwayDepartment")) {
    assert.deepEqual([...mapped.get(type)], type === "AmwayPurchaseRequest" ? ["department", "seller"] : ["department"]);
  }
});
...
test("constructors produce typed objects", () => {
  ...
  assert.equal(createOrder({ department: HASH, idempotencyKey: "k1", customer: PERSON, seller: PERSON, offer: "o1", quantity: 2, lot: "demo-lot-a", facility: "demo-facility", currency: "EUR", unitAmount: 10000, admittedAt: 1 }).quantity, 2);
