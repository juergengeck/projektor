...
test("same-person second instance pairs over the commserver and projects the department", async (t) => {
  ...
  const invite = await deviceA.client.call("amwayLab", "createIoMInvite", {}) as { invitationUrl: string; token: string; person: string };
  assert.equal(invite.person, personA);
  const parsed = new URL(invite.invitationUrl);
  assert.equal(parsed.pathname, "/browser/lab/", "the invitation links back to the lane entry");
...
test("a different person is refused before any network traffic", async (t) => {
  ...
  const invite = await deviceA.client.call("amwayLab", "createIoMInvite", {}) as { invitationUrl: string };
