...
test("buy confirms automatically, decrements inventory, and refuses overselling", async ({ page }, testInfo) => {
  ...
  const actions = header.locator(".lab-header-actions");
  const [headerBox, brandBox, actionsBox] = await Promise.all([
    header.boundingBox(),
    brand.boundingBox(),
    actions.boundingBox(),
  ]);
  if (!headerBox || !brandBox || !actionsBox) throw new Error("desktop workspace header has no layout box");
  const verticalOverlap = Math.min(brandBox.y + brandBox.height, actionsBox.y + actionsBox.height)
    - Math.max(brandBox.y, actionsBox.y);
  expect(verticalOverlap).toBeGreaterThan(0);
  expect(actionsBox.x).toBeGreaterThan(headerBox.x + headerBox.width / 2);
  expect(headerBox.x + headerBox.width - (actionsBox.x + actionsBox.width)).toBeLessThanOrEqual(40);
  ...
  await admin.getByRole("button", { name: "Appoint Manager", exact: true }).click();
  const appointSeller = manager.getByRole("button", { name: "Appoint Seller", exact: true });
  ...
  const shareWithCustomer = seller.getByRole("button", { name: "Share offer-glister-1 down", exact: true });
  await expect(shareWithCustomer).toBeEnabled({ timeout: 90_000 });
  await shareWithCustomer.click();

  const buy = customer.getByRole("button", { name: "Buy 1x (offer-glister-1)", exact: true });
