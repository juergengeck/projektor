...
| 16 | Staleness detection for derived projections. *Direction decided:* arriving evidence is established as current state by `sync.core` (`SyncRule.onImported`, with `onImportedAfterBatch` so state is never half-formed when something evaluates against it); evaluation then re-derives forward and sees the chain that was previously absent. Nothing searches backward from an arriving authority. *Open:* a stored projection cannot currently be told apart from a current one. Its `evaluatedAt` is a self-asserted time, and with no trusted clock a time cannot answer "is this still current" — a reference to the chain-state version it was derived against can, in one comparison. | The "work before you formalize" position in MR-3; any access-control claim |
