...
| 16 | Re-evaluation of pending verification decisions. *Direction decided:* `sync.core`'s `SyncRule.onImported` on the chain-evidence types, with `onImportedAfterBatch` so a chain is evaluated whole rather than part-arrived, and `importBatchContext` since chain evidence is context for later handlers. *Precondition:* pending status objects must be reverse-mapped by the authority they await, or arrival gives no path back to what was waiting and the trigger degrades to a scan. **Enable that reverse map before statuses are written** — it can be enabled later but there is no backfill, so earlier objects stay permanently unindexed. | The "work before you formalize" position in MR-3 |
