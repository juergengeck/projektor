...
| 16 | Trust established on read. *Decided:* arriving evidence is established as current state by `sync.core` (`SyncRule.onImported`, with `onImportedAfterBatch` so state is never half-formed when evaluated against); evaluation re-derives forward and is indifferent to whether the state changed by late arrival or by revocation. A trigger that only re-checked pending decisions would catch verification but never narrowing, which is the direction that fails dangerously. *Open:* the stored projection is currently read rather than re-derived, making it a cache; it should become an audit record. | The "work before you formalize" position in MR-3 |
