...
- Follow-up (reload wedge): reloading the page into persisted worker state wedged CHUM — paired and connected, but members never learned the department and not even new writes arrived; boot then stalls in initializing. Repro: boot 4 node workers, pair, seed, terminate, reboot same dirs (`/tmp/lab-reload-repro.mjs`, scratch, uncommitted). Attempted fix (dial only higher-sorting peers) regressed mesh formation and was reverted. Resolution: the browser boots workers into session-scoped directories (`amway-lab-<key>-<session>`, `worker.ts`/`transport.ts`), so every page load is a fresh boot down the proven pairAll-plus-seed path; stale sessions are pruned best-effort. The committed restart test guards the persistent-worker path instead. Open one.models question (not blocking the demo): why post-restart links report connected yet deliver nothing to some members.
